...
export function OgImage({
  ...
  return (
    <div
      tw="h-full w-full flex flex-col p-11 pb-7 bg-[#fff9ec] text-black"
      style={{
    ...
    >
      <div tw="flex flex-col">
        <span
          tw="text-6xl font-bold pb-5"
          style={{ fontFamily: "Space Grotesk Bold" }}
        >
          {title}
        </span>
        <span tw="text-3xl flex">{description}</span>
      </div>
      <div tw="mt-auto flex-col flex">
        <hr tw="bg-black w-full" />
        <div tw="flex mt-7">
          <span tw="text-3xl flex mr-auto">{note}</span>
          <svg
            viewBox="0 0 251 46"
            fill="none"
