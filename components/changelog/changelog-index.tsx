          ...
          <div className="md:col-span-3">
            <Link key={page.route} href={page.route} className="block group">
              {page.frontMatter?.ogImage ? (
                <div className="mb-14 rounded-xl relative aspect-[16/10] overflow-hidden shadow-md group-hover:shadow-lg">
                  <Image
                    src={page.frontMatter.ogImage}
                    className="object-cover"
                    alt={page.frontMatter?.title ?? "Changelog image"}
                    fill={true}
                    priority={i < 3}
                    unoptimized={page.frontMatter.ogImage?.endsWith(".gif")}
                  />
                </div>
              ) : null}
